...
class Verify {
    ...
    public function entry($id = '') {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length*$this->fontSize*1.5 +25; //+ $this->length*$this->fontSize/2---ltj
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 2;//原来是$this->fontSize * 2.5---ltj
        ...
        for ($i = 0; $i<$this->length; $i++) {
            if($this->useZh) {
				...
				if($i==0)$codeNX += mt_rand($this->fontSize*0.7,$this->fontSize*1.1);//ltj 2016-3-9
                else $codeNX += mt_rand($this->fontSize*1.2, $this->fontSize*1.6);
                // 写一个验证码字符
                $this->useZh || imagettftext($this->_image, $this->fontSize, mt_rand($this->slantRange[0], $this->slantRange[1]), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
