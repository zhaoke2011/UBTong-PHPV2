...
class ShopController extends AdminController {
    ...
    public function reset(){
        ...
        if($reset){
            action_log('store_operation', 重置密码成功, $id, UID);
            $this->success('重置密码成功');
        }else{
            action_log('store_operation', 重置密码失败, $id, UID);
            $this->error('重置密码失败');
        }
    }
    //冻结账号 欧阳龙泉 05.17
    public function freeze(){
        ...
        if($freeze){
            action_log('store_operation', 冻结成功, $id, UID);
            $this->success('冻结成功');
        }else{
             action_log('store_operation', 冻结失败, $id, UID);
            $this->error('冻结失败');
        }
    }
    //取消冻结账号 欧阳龙泉 05.17
    public function nofreeze(){
        ...
        $freeze=M('a_shop')->where('as_id='.$id)->save($data);
        if($freeze){
            action_log('store_operation', 取消冻结成功, $id, UID);
            $this->success('取消冻结成功');
        }else{
            action_log('store_operation', 取消冻结失败, $id, UID);
            $this->error('取消冻结失败');
        }
    }
    //批量冻结 欧阳龙泉 05.17
