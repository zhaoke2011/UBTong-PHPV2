...
class MenuadminAction extends AdminbaseAction {
    ...
    function _getAccessToken(){
	    //实例化数据库类并连接数据库
	    $wxConfig = M('wxConfig');
	    $accessToken = $wxConfig->where("key='WX_ACCESS_TOKEN'")->getField('value');
	    $access = $accessToken ? json_decode($access, true) : array();
	    
	    if($access['expires_in'] < time()){ //已经过期
	    	$appid = $wxConfig->where("key='WX_APPID'")->getField('value');
	    	$appsecret = $wxConfig->where("key='WX_APPSECRET'")->getField('value');
	    	import("Curl");
	    	$curl = new Curl();
	    	...
	    	$data = array(
	    			'key' => 'WX_ACCESS_TOKEN',
	    			'value' => json_encode($access_arr),
	    	);
	    	//保存
	    	if(empty($accessToken)){ //需要创建
	    		$wxConfig->add($data);
	    	}else{ //需要更新
	    		$wxConfig->where("key='WX_ACCESS_TOKEN'")->save($data);
	    	}
    ...
    function add_post(){
    		...
    		$data['event_key'] = $_POST['key'] ? $_POST['key'] : '';
    		$data['status'] = 1;
